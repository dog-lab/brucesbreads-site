---
title: "Double Dutch: Kalamata Olive & Gouda Sourdough"
subtitle: "Bread Flour & Whole Wheat, studded with Kalamata Olives and Aged Gouda"
heroImage: "/images/double-dutch/double_dutch_01_hero.jpg"
section: "savory"
publishDate: 2026-07-14
cardFilePrefix: "Double_Dutch_Kalamata_Gouda"
ddt: "75-78F (24-26C)"
yieldNote: "Total finished dough weight: ~1,188 g, yielding one substantial boule."
hydrationSaltNote: "This loaf runs a bit lower on hydration (68% vs. the 75%+ you'll see in seeded loaves) because olives and cheese both bring their own moisture and fat to the party -- a wetter base dough gets sloppy fast once they're folded in. Salt is also trimmed down from the usual ~2%, since kalamata olives are brined and salty enough to make up the difference on their own."
flatlayImage: "/images/double-dutch/double_dutch_02_flatlay.jpg"
flatlayAlt: "Ingredient flat-lay before mixing"
dough:
  - name: "Bread flour"
    weight: "450 g"
    percent: "90%"
  - name: "Whole wheat flour"
    weight: "50 g"
    percent: "10%"
  - name: "Total flour"
    weight: "500 g"
    percent: "100%"
    isTotal: true
  - name: "Water (main dough)"
    weight: "340 g"
    percent: "68%"
  - name: "Ripe starter (100% hydration)"
    weight: "100 g"
    percent: "20%"
  - name: "Fine sea salt"
    weight: "8 g"
    percent: "1.6%"
  - name: "Total dough weight"
    weight: "948 g"
    isTotal: true
inclusions:
  - name: "Kalamata olives, pitted, roughly chopped"
    weight: "120 g"
    percent: "24%"
    notes: "Pat thoroughly dry after chopping -- excess brine loosens the dough"
  - name: "Aged Gouda, cubed (~1 cm)"
    weight: "120 g"
    percent: "24%"
    notes: "Keep chilled until ready to fold in"
  - name: "Total inclusions"
    weight: "240 g"
    percent: "48%"
    isTotal: true
nonstandardIngredients:
  - ingredient: "Kalamata olives"
    note: "bring both moisture and salt to the dough -- pat them dry after chopping, and the reduced overall salt in this recipe (1.6% instead of the usual ~2%) already accounts for their brininess. Taste one from your jar before mixing; brands vary a fair amount in saltiness."
  - ingredient: "Aged Gouda"
    note: "was chosen over young Gouda specifically because it holds its shape better under heat -- young Gouda is softer and more prone to fully melting and leaking out of the dough during baking."
  - ingredient: "Cheese temperature"
    note: "keep the cheese cold right up until it's folded in. Warm cheese smears through the dough instead of staying in distinct pockets, which costs you both texture and those nice melty pockets in the crumb."
nutritionBasis: "Estimated, USDA-derived -- per slice, 1/12 loaf, ~83 g"
nutritionCaveat: "Not precise lab figures; actual values shift with the specific olives and cheese used."
nutritionFacts:
  - label: "Calories"
    amount: "~210 kcal"
  - label: "Total fat"
    amount: "~9 g"
  - label: "Protein"
    amount: "~7 g"
  - label: "Carbohydrates"
    amount: "~25 g"
  - label: "Dietary fiber"
    amount: "~1.5 g"
  - label: "Sugars"
    amount: "~0.5 g"
  - label: "Sodium"
    amount: "~340 mg"
vitamins:
  - ingredient: "Kalamata olives"
    description: "vitamin E and heart-healthy monounsaturated fat (mostly oleic acid), plus a small iron contribution."
  - ingredient: "Aged Gouda"
    description: "calcium, vitamin B12, and phosphorus -- cheese is a genuinely dense source of all three, even in cube-sized portions."
  - ingredient: "Whole wheat flour"
    description: "B vitamins and a touch of manganese, though at only 10% of the flour blend its contribution here is modest."
nutritionNote: "This loaf runs noticeably saltier and richer than a typical sourdough, between the brined olives and the cheese -- a satisfying trait for a savory table loaf, but worth knowing if you're tracking sodium closely."
equipment:
  - "Digital kitchen scale"
  - "Large mixing bowl"
  - "Bench scraper"
  - "Banneton or bowl lined with a floured cloth"
  - "Lame or sharp blade for scoring"
  - "Dutch oven with lid (450-475F / 230-245C rated -- check your specific pot and knob)"
  - "Ooni Halo Pro Spiral Mixer (optional -- manual method also given in full)"
  - "Instant-read thermometer (optional)"
  - "pH meter (optional)"
playlist:
  - phase: "Starter feeding / quiet rest"
    tracks:
      - artist: "Esbjörn Svensson Trio"
        track: "Strange Place for Snow"
        album: "Strange Place for Snow"
        year: 2004
      - artist: "Pat Metheny Group"
        track: "Minuano (Six Eight)"
        album: "Still Life (Talking)"
        year: 1987
      - artist: "Petros Klampanis"
        track: "Chroma"
        album: "Chroma"
        year: 2017
  - phase: "Active mixing"
    tracks:
      - artist: "Snarky Puppy"
        track: "Lingus"
        album: "We Like It Here"
        year: 2014
      - artist: "Chick Corea Elektric Band"
        track: "Got a Match?"
        album: "The Chick Corea Elektric Band"
        year: 1986
      - artist: "Scope"
        track: "Watch Your Step"
        album: "Scope"
        year: 1974
      - artist: "Mahavishnu Orchestra"
        track: "Birds of Fire"
        album: "Birds of Fire"
        year: 1973
  - phase: "Bulk fermentation (the long wait)"
    tracks:
      - artist: "Hiromi Uehara"
        track: "Seeker"
        album: "Alive"
        year: 2014
      - artist: "Yellowjackets"
        track: "Imperial Strut"
        album: "Yellowjackets"
        year: 1981
      - artist: "Béla Fleck and the Flecktones"
        track: "The Sinister Minister"
        album: "Béla Fleck and the Flecktones"
        year: 1990
  - phase: "Bake and cool"
    tracks:
      - artist: "Chick Corea & Return to Forever"
        track: "Spain"
        album: "Light as a Feather"
        year: 1973
      - artist: "Weather Report"
        track: "Birdland"
        album: "Heavy Weather"
        year: 1977
      - artist: "Herbie Hancock"
        track: "Chameleon"
        album: "Head Hunters"
        year: 1973
booksMovies:
  - groupTitle: "For the craft"
    items:
      - title: "Tartine Bread by Chad Robertson (Chronicle Books, 2010)"
        note: "the sourdough reference that shaped a generation of home bakers."
      - title: "Cooked (Netflix docuseries, 2016, based on Michael Pollan's book)"
        note: "the \"Air\" episode covers bread and fermentation directly."
  - groupTitle: "For the world tour"
    items:
      - title: "Zorba the Greek, dir. Michael Cacoyannis (1964), based on the 1946 novel by Nikos Kazantzakis"
        note: "filmed on location in Crete, a vivid portrait of Greek culture, food, and life."
      - title: "Girl with a Pearl Earring by Tracy Chevalier (1999; 2003 film)"
        note: "set in 17th-century Delft, in the same South Holland region as Gouda's historic cheese trade."
---

## Step-by-Step Tutorial Script

### 1. Feed Your Starter

The night before (or ~8–12 hours before mixing, depending on your kitchen's warmth), feed your starter so it peaks around the time you plan to mix. Look for it to double in size, develop a domed top just starting to flatten at the edges, and smell pleasantly tangy.

<figure><img src="/images/double-dutch/double_dutch_03_starter.jpg" alt="Starter at peak" /></figure>

<div class="callout"><strong>pH check (optional):</strong> a ripe starter typically falls somewhere in the roughly 3.8–4.5 range — a representative range, not a precise target. The dome-and-jiggle visual test is your primary signal.</div>

### 2. Mixing (choose your method)

Salt always goes in **after** the starter, regardless of method.

#### Manual (hand-mixing)

1. Combine the bread flour, whole wheat flour, and the 340 g main dough water in a large bowl. Mix until no dry flour remains. This is your autolyse — rest it, covered, for at least 30 minutes and up to 90–120 minutes if your schedule allows.

   <figure><img src="/images/double-dutch/double_dutch_04_autolyse.jpg" alt="Dough after autolyse, before mixing in starter" /></figure>

2. Add the ripe starter and work it in fully using the pincer or slap-and-fold method until incorporated.
3. Add the salt and continue folding until no dry bits remain and the dough starts to smooth out — typically 3–5 minutes of active work total.
4. Expect only light-to-medium gluten development at this point — that's normal. The stretch-and-folds during bulk fermentation finish the job.

#### Ooni Halo Pro Spiral Mixer

Autolyse is *built into* this method — there's no separate autolyse step ahead of it.

1. Attach the Spiral Dough Hook and Breaker Bar. Add the water to the bowl first, then the flour blend.
2. Initial mix at low speed (roughly 15–20% / approximately 90–100 RPM — a representative range, not a number to dial in precisely) for 2–3 minutes, until no dry flour remains.
3. Rest the dough in the bowl, splash guard closed, for at least 30 minutes and up to 90–120 minutes. This rest *is* the autolyse.
4. Add the ripe starter and mix at a similarly approximate low-to-medium speed range until structure starts to build.
5. Add the salt only once the starter is mixed in.
6. **Inclusion handling:** the olives and cheese get different treatment here. Kalamata olives are sturdy enough to take a brief, low-speed spin in the machine — add them and mix just until distributed, a minute or so. The Gouda is the opposite story: even briefly, machine mixing generates enough friction heat to smear cold cheese cubes through the dough instead of leaving them as distinct pockets, so fold the cheese in **by hand**, after the machine work is done.
7. Finish with a final mix at an approximate medium-speed range to bring the dough to medium gluten development — stop there rather than pushing further. Rest briefly, then hand-fold in the Gouda (and the olives too, if you'd rather keep everything off the machine — that's a fine substitution).

### 3. Bulk Fermentation

If you haven't already folded in the olives (and the Gouda, which always goes in by hand), do that now, gently, distributing both evenly through the dough. Then bulk ferment with a series of stretch-and-folds — 4 folds at 30-minute intervals over the first 2 hours, then left to rise undisturbed.

**Timing:** at a moderate room temperature (~21–23°C / 70–73°F), expect bulk fermentation to take roughly 4–6 hours total. In a warmer kitchen, it'll move faster — watch the dough, not the clock. In a cooler kitchen, expect it to take longer. Look for the dough to have grown by roughly 50–75% in volume and feel jiggly and airy when you nudge the bowl.

<figure><img src="/images/double-dutch/double_dutch_05_bulkfold.jpg" alt="Mid-bulk-fermentation stretch and fold, olives and cheese visible" /></figure>

<div class="callout"><strong>pH check (optional):</strong> dough right after bulk fermentation typically sits in roughly the 3.9–4.3 range.</div>

### 4. Shape

Turn the dough out, pre-shape into a loose round, bench-rest 15–20 minutes, then final-shape into a tight boule. Be a little gentler than usual here — the cheese cubes create weak spots that can tear if you handle the dough too aggressively. Place seam-side up in a floured banneton.

<figure><img src="/images/double-dutch/double_dutch_06_shaped.jpg" alt="Shaped dough with surface tension" /></figure>

### 5. Proof

Proof at room temperature for roughly 2–4 hours, or retard in the refrigerator for 8–16 hours for better flavor and an easier scoring window. The dough should pass a gentle poke-test with a slow, partial spring-back.

### 6. Score and Bake

Turn the dough out onto parchment, score with a confident single cut.

<figure><img src="/images/double-dutch/double_dutch_07_scored.jpg" alt="Scored dough, ready to bake" /></figure>

**Dutch oven preheat:** heat your Dutch oven, empty, to **450–475°F / 230–245°C** — not the pot's rated maximum. Check your specific Dutch oven and lid knob's rated maximum before going higher than this range.

- Bake covered at 450°F / 230°C for 20 minutes.
- Uncover and bake at 425°F / 220°C for another 20–25 minutes, until deeply golden-brown. Keep an eye out for cheese bubbling out at the base of the loaf — a little escaping and crisping up is normal and, frankly, delicious, but a lot suggests the cheese wasn't cold enough going in.
- Internal temperature target: roughly 205–210°F / 96–99°C.

<div class="callout"><strong>pH check (optional):</strong> dough immediately before baking typically sits a touch lower than right after bulk fermentation, roughly 3.7–4.1.</div>

### 7. Cool

Cool on a wire rack for at least 1–2 hours before slicing. This matters even more than usual here — the melted cheese pockets need time to firm back up, or your slices will tear right through them.

<figure><img src="/images/double-dutch/double_dutch_08_crumb.jpg" alt="Crumb, fully cooled and sliced" /></figure>

## Alternatives and Improvements

- **Swap Gouda for feta** for a tangier, crumblier take — feta is also salty, so trim the added salt a touch further if you go this route.
- **Add sun-dried tomatoes** alongside the olives for a fuller Mediterranean flavor profile.
- **Fold in fresh rosemary or thyme** — either pairs naturally with both olives and aged cheese.
- **Try smoked Gouda** in place of aged Gouda for a deeper, smokier flavor throughout the crumb.
- **Swap Kalamata for Castelvetrano olives** for a milder, buttery flavor and noticeably less salt — you'll likely want to bring the added salt back closer to the standard ~2% if you do.

## Troubleshooting / FAQ

<table>
<tr><th>Issue</th><th>Likely Cause</th><th>Fix</th></tr>
<tr><td>Cheese burned or stuck to the parchment/pot</td><td>Gouda wasn't cold enough, or too much leaked out during the uncovered bake</td><td>Keep cheese well-chilled until folding in; tent loosely with foil if it's browning too fast</td></tr>
<tr><td>Olives and cheese sank to the bottom of the loaf</td><td>Inclusions folded in unevenly, or added too early when the dough was still slack</td><td>Fold gently but thoroughly across several turns; make sure bulk fermentation has built enough strength first</td></tr>
<tr><td>Dough tore during shaping</td><td>Cheese cubes create natural weak points</td><td>Shape a little more gently than you would a plain dough</td></tr>
<tr><td>Loaf tastes too salty</td><td>Your olive brand runs saltier than average</td><td>Rinse and pat the olives extra dry, or reduce added salt further next bake</td></tr>
<tr><td>Dense, gummy crumb</td><td>Sliced before fully cooled</td><td>Let the loaf rest a full 2 hours -- the cheese pockets need time to firm back up</td></tr>
</table>

## Ingredient History Story

Kalamata olives take their name from the city of Kalamata, on the Messinian Gulf in the Peloponnese region of southern Greece, where olive cultivation stretches back thousands of years as one of the foundational crops of Mediterranean agriculture.

<figure><img src="/images/double-dutch/double_dutch_09_olive_origin.jpg" alt="Olive grove near Kalamata, Greece" /></figure>

Kalamata olives specifically are large, almond-shaped, and cured in a way that gives them their deep purple color and rich, fruity flavor — distinct enough that "Kalamata" now carries protected geographical status in the European Union, meaning only olives grown in that specific region can legally carry the name.

Gouda cheese, meanwhile, is named for the Dutch city of Gouda in South Holland, which has served as a cheese trading hub since the Middle Ages rather than necessarily being where all "Gouda" cheese is actually produced — much like the city lent its name to a style more than a single point of origin.

<figure><img src="/images/double-dutch/double_dutch_10_gouda_origin.jpg" alt="Historic cheese market scene evoking Gouda, Netherlands" /></figure>

The city's weekly cheese market, with its traditional weighing house and rows of wheels stacked in the square, became a cultural symbol of Dutch dairy trade for centuries. Aging is what separates young Gouda — mild, soft, and springy — from the aged Gouda used in this recipe, which develops a firmer, almost caramel-like character over months or years, along with the small crunchy protein crystals aged-cheese lovers seek out.

Between the two of them, this loaf is a genuine handshake across the Mediterranean and the North Sea — Greek brine meeting Dutch dairy tradition in a single Dutch oven, which is where the name comes from twice over.

**An interesting story:** the same weighing houses used for centuries in the Gouda cheese trade are still standing in the city center today, and the traditional cheese market is still reenacted there in summer, complete with period costumes and the same hand-clapping price-negotiation ritual traders used hundreds of years ago.

## Bloopers Section

Even a well-tested recipe goes sideways sometimes — here's what that can look like, and the one-line fix for each.

**The cheese moat.** A ring of melted Gouda pools and crisps up around the base of the loaf instead of staying tucked inside — dramatic-looking, and honestly not the worst outcome, but a sign the cheese wasn't cold enough going in. The fix: keep those cubes in the fridge right up until the moment you fold them in.

<figure><img src="/images/double-dutch/double_dutch_11_blooper_moat.jpg" alt="Melted cheese pooled around the base of the loaf" /></figure>

**The sunken bottom.** Slice into the loaf and all the olives and cheese have settled into a dense, wet layer at the bottom instead of spreading through the crumb. The fix: fold more thoroughly, and make sure the dough has built real strength during bulk fermentation before you ask it to hold onto that much weight.

<figure><img src="/images/double-dutch/double_dutch_12_blooper_sunken.jpg" alt="Crumb cross-section showing inclusions settled at the bottom" /></figure>

**The scorched top.** Between the uncovered bake stage and any cheese that escaped early, the top can go from golden to genuinely burnt faster than you'd expect. The fix: check a few minutes early on your first bake of this recipe, and tent with foil the moment it looks close.

<figure><img src="/images/double-dutch/double_dutch_13_blooper_scorched.jpg" alt="Over-darkened, scorched loaf top" /></figure>

None of this means you did something wrong — it means you're actually baking. Every one of these is a normal, forgettable part of learning a new recipe, not a reason to give up on it.
